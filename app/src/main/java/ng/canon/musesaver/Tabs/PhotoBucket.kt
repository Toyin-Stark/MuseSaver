...
import ng.canon.musesaver.GlideApp
...
class PhotoBucket : Fragment(),SwipeRefreshLayout.OnRefreshListener {
    ...
    fun FileQuery() {
        ...
        arrayList!!.clear()
        path = Environment.getExternalStorageDirectory().absolutePath + "/musesavers/"
        val dex = File(Environment.getExternalStorageDirectory().absolutePath, "musesavers")
        if (!dex.exists())
            dex.mkdirs()
